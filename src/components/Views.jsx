    ...
    return (
        <div className="popup-container">
            <form className="popup-wrapper g2" onSubmit={handleSumbit}>
                <label htmlFor="view__name" className="row">
                    当前方案：
                    <input
                        value={viewName}
                ...
                <div className="row  g1 flex-center">
                    <input
                        type="button"
                        name="cancel"
                        className="white small bordered"
                        onClick={closePopup}
                        value="取消"
                    />
                    <input
        ...
        ${isSelected ? 'selected' : 'bordered'}`}
            ...
            >
                <AutoWidthInput
                    ...
                    readOnly={readOnly}
                />
                <button onClick={async (e) => await handleDelete(e, id)}>
                    <CloseIcon />
                </button>
            </div>
        )
...
const Views = ({ views, editable, selected, setSelected }) => {
    const updateTableData = useUpdateTableDataContext()
    const updateTableStates = useUpdateTableStatesContext()
    const { alertError, alertSuccess, alertWarning, alertConfirm } =
        useAlertContext()
    const states = useTableStatesContext()
    const { updateSelectedQuery, resetSelectedQuery, selectedQuery } =
        useSelectedDataContext()
    ...
    setSelected(defaultSelection.viewId)
    ...
    const handleSave = async () => {
        if (
            defaultSelection.viewId === null ||
            defaultSelection.viewId === undefined ||
            defaultSelection.viewId === ''
        ) {
            alertWarning('未选择方案！')
            ...
            let res
            const cols = visibleCols.map((col, i) =>
                getColParams(col, i + 1, defaultSelection.filterCriterias)
            ) //TODO: Adding sequence logic

            const params = {
                tableId: defaultSelection.tableId,
                viewName: newViews.find(
                    (view) => view.viewId === defaultSelection.viewId
                ).viewName,
    ...
    async function confirmDeleteView(id) {
        ...
        switch (res.code) {
            case 200:
                ...
                setNewViews((prev) =>
                    prev.filter((value) => value.viewId !== id)
                )
                break
            case 400:
                alertError(res.message)
    ...
    const handleViewClick = async (id) => {
        ...
        const res = await fetchData({
            ...
            viewId: id,
            filterCriterias: [],
            secTab: defaultSelection.secTab
        })
    ...
    const handleViewNameChange = (newName, viewId) => {
        setNewViews((prev) => [
            ...prev.filter((view) => view.viewId !== viewId),
            { viewId, viewName: newName }
        ])
    }
        ...
        <>
            <div className="view-container row">
                ...
                {editable && (
                    <div className="controls row g1">
                        ...
                        <button className="rounded white" onClick={handleSave}>
                            <SaveIcon />
                            保存方案
                        </button>
                    </div>
            ...
            {visibleCols?.length > 0 ? (
                <div className="col flex-center">
                    <Filters headers={visibleCols} display={true} />
                </div>
            ) : (
