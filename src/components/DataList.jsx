...
const inquiryTypeOptions = [
    { inquiryType: 'PO(客户付款)' },
    { inquiryType: 'PR(客户提出付款意向)' },
    { inquiryType: 'YG(供应链预估)' },
    { inquiryType: 'YC(销售预测)' },
    { inquiryType: 'XD(意向询单)' }
]
...
const DataList = memo(function DataList({
    ...
    const handleSelect = (option) => {
        if (identifier === 'itemCode') {
            ...
            handleChange(
                ['itemCode', 'itemName', 'itemType', 'itemId'],
                [option.itemCode, option.itemName, option.itemType, option.id]
            )
    ...
    return (
        <div className="data-list" ref={containerRef}>
            ...
            {showDropdown && options && (
                <ul className="data-list-dropdown">
                    {(identifier === 'itemCode' ||
                        identifier === 'materialCode') && (
                        <li className="row sticky g1">
                            <div style={{ width: 66 }}>物料编码</div>
                            <div>物料名称</div>
                        </li>
                    )}
                    {options.length > 0 ? (
