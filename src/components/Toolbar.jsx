...
import {
    getIndexes,
    EngToCn,
    VisibilityToHeadersENG,
    snakeToCamelCase
} from '../js/transformType'
...
export default function Toolbar({
    ...
    const { alertSuccess, alertError, alertConfirm, alertWarning } =
        useAlertContext()

    const { selectedQuery, setSelectedData, setSavedNewData } =
        useSelectedDataContext()
    ...
    const handleDelete = async () => {
        if (!noRowSelected())
            alertConfirm('确定删除选定的信息？', async () => {
                if (!noRowSelected()) {
                    const orderIds = getIndexes(rowSelection)?.map(
                        (index) => tableData[index].inquiry_id
                    )
                    const res = []
                    orderIds?.forEach(async (orderId, i) => {
                        res[i] = await deleteInquiry(orderId)
                    })

                    if (
                        res.some((item) => item.code === 400 || item.code === 1)
                    ) {
                        alertError(
                            '删除失败！失败原因：' +
                                [
    ...
    const handleExport = () => {
        if (noData(tableData) || noVisibleCols(states.columnVisibility)) {
            ...
            alertConfirm('确定导出该表单？', () => {
                ...
                const headers_ENG = VisibilityToHeadersENG(
                    states.columnVisibility
                )
                ...
                const filename = children.filter(
                    (child) => child.path === activeTab
                )[0].name
    ...
    const handleStartInquiry = async () => {
        if (!noRowSelected()) {
            ...
            let newInquiries = inquiries.map((obj) => ({
                inquiryId: obj.inquiry_id
            }))
    ...
    const handlePin = async () => {
        ...
        try {
            ...
            const results = await Promise.all(
                pinnedIndexes.map(async (index) => {
                    const materialCode = tableData[index].materialCode
                    try {
                        const response = await setTop({
                            topMaterialCode: materialCode
                        })
                        return { materialCode, response }
                    } catch (error) {
                        return { materialCode, error }
                    }
                })
            )

            // 处理结果
            const alreadyPinned = results.filter(
                (result) =>
                    result.response?.code === 400 &&
                    result.response?.message === '物料已置顶'
            )
            ...
            console.error('置顶操作出错:', error)
            alertError('置顶操作失败，请稍后重试')
        }
    }
            ...
            <div className="row toolbar">
                <div className="row flex-center">
                    <ToolbarButton
                        ...
                        handler={handleNew}
                        text="新增"
                        additionalCondition={auth.userType != '3'}
                    />
                    <ToolbarButton
                        feature="delete"
                    ...
                    />
                    <ToolbarButton
                        feature="startInquiry"
                        handler={handleStartInquiry}
                        text="开始询单"
                        additionalCondition={auth.userType != '3'}
                    />
                    <ToolbarButton
                        feature="allowInquiry"
                        handler={handleAllowInquiry}
                        text="允许询单"
                        additionalCondition={auth.userType == '1'}
                    />
                </div>
                ...
                {features.includes('visibility') && !noData(tableData) && (
                    <div className="row flex-center status">
                        <ColVisibility
                            editable={
                                tableId === 1 && defaultSelection.viewId > 0
                            }
                        />
                    </div>
                )}
