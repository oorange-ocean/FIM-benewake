...
const AdminTable = ({
    ...
    const deleteData = async () => {
        ...
        const deleteList = data.filter((_, index) =>
            selectedRows.includes(index)
        )
        const payloads =
            type === 'materialType'
                ? deleteList.map((obj) => obj.itemId)
                : deleteList.map((obj) => obj.id)
        // map 每个 payload，走 deleteAdminData 接口
        const messages = []
        const promises = payloads.map((payload) =>
            deleteAdminData(type, payload)
        )
        ...
        Promise.all(promises)
            .then((results) => {
                results.forEach((res, index) => {
                    if (
                        res.message.includes('失败') ||
                        res.message.includes('异常')
                    ) {
    ...
    const handleDelete = async () => {
        ...
        let payloads
        const deleteList = rows.filter((_, index) =>
            selectedRows.includes(index)
        )
        ...
        const promises =
            type === 'inquiryType'
                ? [deleteAdminData(type, payloads)]
                : payloads.map((payload) => deleteAdminData(type, payload))
            ...
            .then((results) => {
                results.forEach((res, index) => {
                    if (
                        ...
                        messages.push(
                            `${JSON.stringify(payloads[index])} ${res.message}`
                        )
                    }
                })
    ...
    const handleDeleteRow = () => {
        if (
            rows.filter((_, index) => selectedRows.includes(index)).length === 0
        ) {
            alertWarning('未选择数据！')
        } else {
                    ...
                    <div className="tbody">
                        {rows.map((row, i) => (
                            <Row
                                ...
                                setEditable={(value) =>
                                    setEditableRowIndex(value ? i : null)
                                }
                            />
                        ))}
                    </div>
