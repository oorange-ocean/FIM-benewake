...
export default function InventoryOccupy() {
    ...
    const { pageSize, setPageSize, filters, setFilters } =
        usePageState('inventoryOccupy')
    ...
    const handleSearch = async (filters) => {
        const response = await getByMaterialCode({
            materialCode: filters[0].value
        })
        if (response.data) {
    ...
    const features = ['pin', 'unpin', 'export', 'refresh']
