...
const Manage = ({ type }) => {
    ...
    useEffect(() => {
        ...
        if (data.length > 0) {
            const initialSchema = Object.keys(data[0] || {}).flatMap((key) =>
                adminDefs.filter((item) => item.eng === key)
            )
            setSchema(initialSchema)

            if (prevTypeRef.current !== type) {
                setFilters([
                    { key: initialSchema[0].eng, condition: 'like', value: '' }
                ])
    ...
    const handlePageChange = async (page, size) => {
        ...
        if (isFiltered) {
            ...
            const data = await fetchAdminData(adminSchema[type].select, {
                page,
                size
            })
